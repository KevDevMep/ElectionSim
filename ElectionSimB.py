...
def filter():
    if loaded.get():
        ...
        class_ = set([dem.get(), rep.get(), comp.get()])
        if fileType.get():
...
tk.Button(root, text='Select All', command=selectAll).grid(row=11, column=1)
tk.Button(root, text='Select None', command=selectNone).grid(row=11, column=2)
...
tk.Checkbutton(root, variable=comp, text='Comp', onvalue='Comp', offvalue='').grid(row=9, column=3)
tk.Checkbutton(root, variable=rep, text='Rep', onvalue='R', offvalue='').grid(row=10, column=3)
tk.Button(root, text='Filter', command=filter).grid(row=11, column=3)
...
tk.Label(root, text='Column to Drop').grid(row=8, column=4)
tk.Entry(root, textvariable=col).grid(row=9, column=4)
tk.Button(root, text='Drop', command=drop).grid(row=10, column=4)
...
tk.Label(root, text='Export Tag').grid(row=8, column=5)
tk.Entry(root, textvariable=exportTag).grid(row=9, column=5)
tk.Button(root, text='Columns', command=columns).grid(row=10, column=5)
